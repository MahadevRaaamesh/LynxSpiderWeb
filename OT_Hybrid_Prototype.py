...
MODEL_SAVE_PATH = "ot_hybrid_dqn.pt"
...
class HybridCoordinator:
    ...
    def run_epoch(self, steps=1000, train=True):
        ...
        for step in range(steps):
            actions = {tid: self.agents[tid].select_action(states[tid], train) for tid in self.tls_ids}
            
            # Apply actions with safety guards
            for tid in self.tls_ids:
                if actions[tid] == 1: # Switch
                    # Guard: Don't switch if in yellow phase or just started green
            ...
            # OT computation (every 5 steps for stability)
            if step % 5 == 0:
            ...
            for i, tid in enumerate(self.tls_ids):
                ...
                # R = -LocalDelay - OT_Deviance - SwitchPenalty
...
def main():
    ...
    # Temporary C to initialize, will be updated in run_epoch
    engine = OTEngine(np.eye(4)) 
    
    coordinator = HybridCoordinator(sim, graph, engine)
    
    print("\n--- Phase 1: Training Hybrid Agent (5 Episodes) ---")
    for ep in range(5):
        wd, wait = coordinator.run_epoch(steps=1000, train=True)
        print(f"Episode {ep} finished. Avg Halting: {wait:.2f}")

    print("\n--- Phase 2: Final Demonstration (2000 steps) ---")
    wd, final_avg = coordinator.run_epoch(steps=2000, train=False)
    print("\n" + "="*40)
    print(f"HYBRID SIMULATION FINISHED")
